...
# 처음 잡을 때의 근거(2026-09-14 03:14~13:34 UTC, 상태 파일 1,429판)는 '한 실행 최대
# 4건이니 3배로 잡으면 평소에는 안 닿는다'였습니다. **그 전제는 틀렸습니다.**
#
# 실측(2026-09-15 16:18 ~ 09-17 11:25 UTC, 43.3시간, 커밋 꼬리표 360개):
#   - 물어볼 자리 836건 중 **30건을 아예 못 물어봤습니다**(3.6%). 전부 이 상한입니다
#     (눈금 불신 0회 / 시간 상한 0회 — `scripts/commit_notes.py`가 갈라 찍습니다).
#   - 한 실행 최대는 4건이 아니라 **30건**이었습니다(2026-09-16 18:15 UTC).
#     정지 뒤 밀림이 아닙니다 — 같은 구간 상태 커밋 간격은 53~92초입니다.
#   - **"넘친 것은 다음 실행에서 다시 본다"도 성립하지 않았습니다.** 넘친 자리는
#     보류로 남는데 그 매물이 검색 창에서 밀려나면 다시 물을 일이 없습니다. 실측에서
#     18건이 17.3시간 동안 `checked_at`이 안 움직인 채 굳었고, 앞선 3건은 그대로
#     24시간 TTL로 사라졌습니다. 보류는 `seen`에도 안 들어갑니다.
#
...
# 큽니다 — 값이 매번 달라 같은 제목끼리 걸려 있던 델타가 끊기는 몫입니다). 붙는 실행은
# **43.3시간 실측으로 조회 단계 커밋의 16.0%**(360/2,247)라 하루 약 200개, **12KB**로
# 저장소 하루 증가분(9.67MB)의 **0.12%**입니다.
#
# 이 자리에 '38%'가 적혀 있었는데 배포 직후 **1.8시간**만 보고 쓴 값입니다. 며칠치로
# 재니 16.0%입니다 — 배포 전 추정 12.6%보다 오히려 더 틀린 값이 '실측'이라는 이름으로
# 앉아 있었습니다. 짧은 창에서 나온 비율은 실측이어도 결론에 쓰지 않습니다.
#
    ...
    for fields in sorted(items, key=state_update_order):
        ...
        if record is None:
            ...
            if isinstance(last_alert_price, int) and isinstance(price, int):
                ...
                # 아직 기준가가 없는 매물에만 지금 가격으로 기준을 세웁니다.
                last_alert_price = price
            # else: 이번 관측에서 가격을 알 수 없습니다(가격 비공개 등).
            #   예전에는 여기서도 last_alert_price = price를 해서 기준가를 None으로 지웠는데,
            #   그러면 다음 관측의 가격이 그대로 새 기준가가 되면서 **기준가가 위로 올라갑니다.**
            #   사용자에게는 이미 더 싼 가격을 알린 뒤인데 기준만 비싸게 되돌아가는 것이라,
            #   그 다음 인하 알림이 '¥(올라간 기준가) → ¥(지금 가격)'이라는 틀린 문장을 보냅니다.
            #   실제로 그렇게 나갔습니다(README "가격 인하 판정" 참고). 모르는 값은 기준을
            #   건드리지 않는 것이 맞습니다.
